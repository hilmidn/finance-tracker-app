...
import { Home, Users, Tag, LogOut, ChevronRight, Edit2, Check, X as XIcon } from 'lucide-react'
...
export default function HouseholdSettingsPage() {
  ...
  const { household, status, role, isMember, hasPendingInvite, loading, renameHousehold, leave } = useHousehold(userId)
  ...
  if (!loading && !household) {
    return (
      <div className="text-center py-16 space-y-3">
        ...
        <p className="text-gray-400 text-xs">Buat household dari halaman Pengaturan, atau terima undangan yang masuk</p>
      </div>
    )
  }
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
          <p className="text-sm text-amber-900 font-medium">Kamu punya undangan pending</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-r from-indigo-500 to-violet-500 rounded-2xl p-5 text-white shadow-lg">
        <div className="flex items-center gap-3 mb-3">
          ...
          {!editing && isAdmin && (
            <button onClick={() => { setNewName(household.name); setEditing(true) }}
              className="p-2 bg-white/20 rounded-lg hover:bg-white/30 transition-colors">
              <Edit2 size={14} />
            </button>
          )}
        </div>
        <p className="text-[10px] text-indigo-100/80">ID: {householdId?.substring(0, 8)}...</p>
      </div>
      ...
      <div className="flex rounded-xl overflow-hidden border border-gray-200 bg-gray-50 p-1">
        <button onClick={() => setActiveTab('members')}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-medium rounded-lg transition-all ${
            activeTab === 'members' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'
          }`}>
          <Users size={16} /> Member
        </button>
        <button onClick={() => setActiveTab('categories')}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-sm font-medium rounded-lg transition-all ${
            activeTab === 'categories' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500'
          }`}>
