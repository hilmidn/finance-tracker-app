    ...
    <div className="space-y-5">
      ...
      <div className="flex rounded-xl overflow-hidden border border-gray-200 bg-gray-50 p-1">
        <button
          ...
          className={`flex-1 py-2.5 text-sm font-medium rounded-lg transition-all ${
            activeTab === 'pengeluaran' ? 'bg-white text-red-500 shadow-sm' : 'text-gray-500'
          }`}
        >
          Pengeluaran
        </button>
        <button
          ...
          className={`flex-1 py-2.5 text-sm font-medium rounded-lg transition-all ${
            activeTab === 'pemasukan' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500'
          }`}
        >
          Pemasukan
        </button>
            ...
            <div key={cat.id} className="flex items-center justify-between px-4 py-3.5">
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${
                  activeTab === 'pengeluaran' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'
                }`}>
                  {cat.name[0]}
                </div>
      ...
      {showAdd ? (
        <div className="flex gap-2">
          ...
          <button
            onClick={handleAdd}
            className="bg-indigo-600 text-white px-4 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors"
          >
            Simpan
          </button>
          <button
            onClick={() => { setShowAdd(false); setNewCat('') }}
            className="px-3 rounded-xl text-sm font-medium text-gray-500 hover:bg-gray-100 transition-colors"
          >
            Batal
          </button>
        </div>
